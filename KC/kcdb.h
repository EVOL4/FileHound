 ...
 */
	...
	public:
		...
		public:
			//ºÚµ•µƒ÷ÿ‘ÿ
			bool set(unsigned __int64 key, const char* value,size_t len) {
				_assert_(true);
				return set((const char*)&key, sizeof(unsigned __int64), value, len);
			}
			char* get(unsigned __int64 key, size_t* sp)
			{
				_assert_(true);
				return get((const char*)&key, sizeof(unsigned __int64), sp);
			}
			
			bool add(unsigned __int64 key, const char* value, size_t len) {
				_assert_(true);
				return add((const char*)&key, sizeof(unsigned __int64), value, len);
			}

			bool remove(unsigned __int64 key){
				_assert_(true);
				return remove((const char*)&key, sizeof(unsigned __int64));
			}

			int32_t check(unsigned __int64 key) {
				return check((const char*)&key, sizeof(unsigned __int64));
			}
